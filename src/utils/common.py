...
def overwrite_partition_ext(input_df, catalog_name, db_name, table_name, partition_column):
    output_df = re_arrange_partition_column(input_df, partition_column)
    ...
    if spark.catalog.tableExists(f"{catalog_name}.{db_name}.{table_name}"):
        ...
        output_df.write.mode("overwrite").partitionBy(partition_column).format("delta").option('path', f"abfss://container@storageAccount.dfs.core.windows.net/{catalog_name}/{db_name}/{table_name}").saveAsTable(f"{catalog_name}.{db_name}.{table_name}")
